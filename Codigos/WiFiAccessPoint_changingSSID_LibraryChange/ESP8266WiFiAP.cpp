 ...
 */
bool ESP8266WiFiAPClass::softAP(const char* s, const char* passphrase, int channel, int ssid_hidden) {
...
bool ESP8266WiFiAPClass::softAPchangeSsid(const char* newssid) {
bool ret;
    struct softap_config conf;
	struct softap_config conf_current;
	
    wifi_softap_get_config(&conf_current);
	Serial.println(reinterpret_cast<char*>(conf_current.ssid));
	
	strcpy(reinterpret_cast<char*>(conf_current.ssid), newssid);
	
    ETS_UART_INTR_DISABLE();
    if(WiFi._persistent) {
        ret = wifi_softap_set_config(&conf_current);
    } else {	
        ret = wifi_softap_set_config_current(&conf_current);
    }
    ETS_UART_INTR_ENABLE();
